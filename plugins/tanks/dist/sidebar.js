...
async function run() {
    ...
    async function updateMove() {
        ...
        let currentTank = await rtb.board.getById(tank.id);
        if (!currentTank) {
            document.querySelector('#you-are-dead').style.display = 'block';
            return;
        }
        const STEP = 25;
        let move = true;
    ...
    async function createBulletAndRunAnimation() {
        async function rs(resolve) {
            ...
            let currentStep = 1;
            let t = await rtb.board.getById(tank.id);
            let bulletX = t.x;
            let bulletY = t.y;
...
async function getBulletIntersection(bullet) {
...
async function willTankIntersectsWithWall(tankBounds) {
    let widgets = await rtb.board.widgets.get();
    return widgets
        .filter((w) => w.type === 'SHAPE' && (w.text === 'D' || w.text === 'N'))
        .some(block => isObjectsIntersect(tankBounds, block.bounds));
}
function isObjectsIntersect(objectA, objectB) {
...
function showClickOnSidebarMessage() {
    document.querySelector('#click-for-focus').style.display = 'block';
}
function hideClickOnSidebarMessage() {
    document.querySelector('#click-for-focus').style.display = 'none';
}
