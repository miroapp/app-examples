...
function inviteUser(email, account) {
  ...
  return fetch(`https://api.miro.com/v1/accounts/${account.id}/invite`, options)
      .then(res => res.json())
}
...
function findConnectionByEmail(email, account) {
  ...
  return fetch(`https://api.miro.com/v1/accounts/${account.id}/user-connections?fields=user{name,email}&limit=100`, options)
      .then(res => res.json())
...
function deleteUserConnection(connection) {
  ...
  return fetch(`https://api.miro.com/v1/account-user-connections/${connection.id}`, options)
      .then(res => res.ok)
}